...
func RoundToNearest(t Time, r Rounding) Time {
	...
	midnight, _ := NewTime(0, 0)
	roundedTime, err := midnight.Plus(NewDuration(0, roundedMidnightOffset))
	if err != nil {
		// This is the special case where we can’t round up after `23:59>`.
		maxTime, _ := NewTimeTomorrow(23, 59)
		return maxTime
	}
	return roundedTime
}
