...
func TestSerialiseFullBlownRecord(t *testing.T) {
	json := ToJson(func() []Record {
		r := NewRecord(Ɀ_Date_(2000, 12, 31))
		r.SetSummary(NewSummary("Hello #World"))
		r.SetShouldTotal(NewDuration(7, 30))
		r.AddDuration(NewDuration(2, 3), NewSummary("#some #thing"))
		r.AddRange(Ɀ_Range_(Ɀ_TimeYesterday_(23, 44), Ɀ_Time_(5, 23)), NewSummary())
		r.StartOpenRange(Ɀ_TimeTomorrow_(0, 28), NewSummary("Started #todo"))
		return []Record{r}
	}(), nil, false)
	assert.Equal(t, `{"records":[{`+
		`"date":"2000-12-31",`+
