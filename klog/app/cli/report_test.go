...
2022-10-30
	8h
`)._Run((&Report{
		FilterArgs: args.FilterArgs{Date: klog.Ɀ_Date_(2022, 10, 31)},
		Fill:       true,
	}).Run)
	require.Nil(t, err)
	assert.Equal(t, "", state.printBuffer)
...
2021-01-19
	5m
`)._SetNow(2021, 3, 4, 0, 0)._Run((&Report{WarnArgs: args.WarnArgs{NoWarn: true}}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
                       Total
2019 Dec    Sun  1.       0m
...
2018-07-09 (19m!)
`)._Run((&Report{DiffArgs: args.DiffArgs{Diff: true}}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
                       Total    Should     Diff
...
2018-07-09 (19m!)
`)._Run((&Report{DiffArgs: args.DiffArgs{Diff: true}, DecimalArgs: args.DecimalArgs{Decimal: true}}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
                       Total    Should     Diff
...
2019-01-08 (19m!)
`)._Run((&Report{AggregateBy: "week", DiffArgs: args.DiffArgs{Diff: true}, Fill: true}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
                 Total    Should     Diff
...
2019-01-01 (19m!)
`)._Run((&Report{AggregateBy: "quarter", DiffArgs: args.DiffArgs{Diff: true}, Fill: true}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
           Total    Should     Diff
2018 Q1       8h       8h!       0m
...
2019-01-01 (19m!)
`)._Run((&Report{AggregateBy: "month", DiffArgs: args.DiffArgs{Diff: true}, Fill: true}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
            Total    Should     Diff
2018 Feb       8h       8h!       0m
...
2019-01-01 (19m!)
`)._Run((&Report{AggregateBy: "year", DiffArgs: args.DiffArgs{Diff: true}, Fill: true}).Run)
	require.Nil(t, err)
	assert.Equal(t, `
        Total    Should     Diff
2016       8h       8h!       0m
...
2025-01-15
	31h15m
`)._Run((&Report{Chart: true, AggregateBy: "w", WarnArgs: args.WarnArgs{NoWarn: true}}).Run)
		require.Nil(t, err)
		assert.Equal(t, `
                 Total                                                   
2025  Week  1      40h  ▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇         
...
2025-03-01
	126h15m
`)._Run((&Report{Chart: true, AggregateBy: "m", WarnArgs: args.WarnArgs{NoWarn: true}}).Run)
		require.Nil(t, err)
		assert.Equal(t, `
            Total                                                       
2025 Jan     173h  ▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇         
...
2025-07-01
	237h45m
`)._Run((&Report{Chart: true, AggregateBy: "q", WarnArgs: args.WarnArgs{NoWarn: true}}).Run)
		require.Nil(t, err)
		assert.Equal(t, `
           Total                                                    
2025 Q1     316h  ▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇          
...
2027-01-01
	1189h15m
`)._Run((&Report{Chart: true, AggregateBy: "y", WarnArgs: args.WarnArgs{NoWarn: true}}).Run)
		require.Nil(t, err)
		assert.Equal(t, `
        Total                                         
2025    1735h  ▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇        
...
2025-01-16 (1h!)
	45m
`)._Run((&Report{Chart: true, DiffArgs: args.DiffArgs{Diff: true}}).Run)
		require.Nil(t, err)
		assert.Equal(t, `
                       Total    Should     Diff        
